  ...
  if (tab === 'login') {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (!location.pathname.startsWith('/admin')) location.href = '/account';
      } catch (ex) {
...
async function renderAuthArea() {
  ...
  if (user) {
    area.innerHTML = `<a href="/account" id="accountLink" class="btn" style="text-decoration:none;border:1px solid #333;padding:8px 10px;border-radius:10px;color:inherit;background:#111">Account</a>`;
  } else {
    area.innerHTML = `<button id="openAuth" class="btn" title="Login or Sign up">Login</button>`;
